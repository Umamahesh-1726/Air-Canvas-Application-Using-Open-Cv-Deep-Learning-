...
const Sheet: React.FC = () => {
  ...
  const saveToHistory = () => {
    ...
    try {
      ...
      setDrawingHistory([...historyRef.current]);
  ...
  const performUndo = () => {
    ...
    const targetImage = historyRef.current[newStep];
  ...
  const detectGesture = (landmarks: any[]) => {
    ...
    if (indexUp && !middleUp && !ringUp && !pinkyUp && !thumbOut) {
      ...
      return "BrushIncrease";
    }
    else if (!thumbOut && !indexUp && !middleUp && !ringUp && pinkyUp) {
      return "BrushDecrease";
    }
    else if (!indexUp && !middleUp && !ringUp && !pinkyUp) {
      return "Clear";
    }
  ...
  useEffect(() => {
    ...
    hands.onResults((results) => {
      ...
      if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
        ...
        const detectedGesture = isDrawingRef.current ? detectGesture(landmarks) : "None";
        ...
        drawLandmarks(cameraCtx, landmarks, { color: "#FF0000", lineWidth: 1 });
        cameraCtx.restore();
        ...
        if (!smoothPosition.current) {
          ...
          smoothPosition.current.x = alpha * rawX + (1 - alpha) * smoothPosition.current.x;
          smoothPosition.current.y = alpha * rawY + (1 - alpha) * smoothPosition.current.y;
        }
        ...
        if (detectedGesture === "Clear") {
          gestureFrameCount++;
          if (gestureFrameCount >= GESTURE_THRESHOLD && lastGestureAction !== "Clear") {
          ...
          gestureFrameCount++;
          if (gestureFrameCount >= GESTURE_THRESHOLD && lastGestureAction !== "Undo") {
            performUndo();
          ...
          gestureFrameCount++;
          if (gestureFrameCount >= GESTURE_THRESHOLD && lastGestureAction !== "ColorChange") {
            const colors = ["blue", "red", "green", "black", "orange", "purple", "yellow"];
          ...
          gestureFrameCount++;
          if (gestureFrameCount >= GESTURE_THRESHOLD && lastGestureAction !== "BrushIncrease") {
          ...
          gestureFrameCount++;
          if (gestureFrameCount >= GESTURE_THRESHOLD && lastGestureAction !== "BrushDecrease") {
          ...
          if (!lastPosition.current) {
            drawCtx.beginPath();
            drawCtx.arc(canvasX, canvasY, brushSizeRef.current / 2, 0, 2 * Math.PI);
          ...
          if (lastPosition.current !== null && detectedGesture !== "Clear" && detectedGesture !== "Undo") {
            saveToHistory();
            strokeCount = 0;
          }
          
          if (detectedGesture !== lastGestureAction) {
            gestureFrameCount = 0;
          }
          lastPosition.current = null;
        }
      ...
      <div className="w-[35%] flex flex-col p-4 gap-4 bg-gradient-to-b from-slate-800/50 to-slate-900/50">
        <div className="relative h-[60%] overflow-hidden bg-black shadow-2xl rounded-2xl">
          ...
          <div className="absolute px-3 py-2 shadow-lg top-3 left-3 bg-emerald-600/90 backdrop-blur-md rounded-xl">
            <div className="text-xs font-semibold text-emerald-100">Gesture</div>
        ...
        <div className="flex flex-col gap-3 p-4 shadow-xl bg-slate-800/70 backdrop-blur-md rounded-2xl">
          <div className="flex justify-center gap-2">
            <button
              ...
              className={`w-24 px-3 py-2 text-sm font-bold text-white rounded-lg shadow-md transition-all ${
                isDrawing ? 'bg-green-500 hover:bg-green-600' : 'bg-gray-500 hover:bg-gray-600'
              }`}
            >
          ...
          <div className="pt-2 mt-2 border-t border-slate-600">
            <div className="mb-2 text-xs font-semibold text-center text-gray-300">Color Palette</div>
            <div className="flex justify-center gap-2">
              {colors.map((color) => (
                <div
                  key={color}
